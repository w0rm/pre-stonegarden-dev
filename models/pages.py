...
from template import smarty, sanitize, template_global
...
def page_where(with_auth_check=True):
    if auth.get_user() and with_auth_check:
        return ""
    else:
        return " AND is_published AND published_at <= CURRENT_TIMESTAMP"
...
def get_page_by_id(page_id):
    return db.select(
        "pages",
        locals(),
        where="id = $page_id AND NOT is_deleted" + page_where(),
        limit=1)[0]
...
def get_pages_by_parent_id(parent_id):
    return db.select(
        "pages",
        locals(),
        where="parent_id=$parent_id AND NOT is_deleted" + page_where()
    ).list()
